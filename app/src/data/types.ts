...
export type Notice = {
  ...
  mine: boolean
  /**
   * 🆕 我能不能撤下这一条 —— **服务端算的**（`自己发的 || is_school_admin()`）。
   *
   * 🔴 前端**只照它摆按钮**，不许在这里另判角色：2026-10-07 之前界面用的是 `mine`，
   *    于是最高管理员与教务处在别人的通知上看不到「撤下」，而服务端明明允许
   *    （"服务端允许、前端没摆按钮"，第二次；第一次是开学准备页）。
   * ⚠️ 可选：老服务端 / 本地演示夹具没有这个字段 → `undefined` = **不摆**。
   */
  canRevoke?: boolean
  /**
   * 🆕 我能不能给这一条**置顶 / 取消置顶** —— **服务端算的**（`is_school_admin()`，
   * 教务处 ∪ 超管；与 `canRevoke` 是**两个**许可，别合成一个）。
   *
   * 🔴 与 `canRevoke` 同一条纪律：前端**只照它摆按钮**，不许在这里另判角色。
   * ⚠️ 它只说"许可"：**这一条现在是不是置顶的**看 `pinned`（按钮文字由它决定）。
   * ⚠️ 可选：老服务端 / 本地演示夹具没有这个字段 → `undefined` = **不摆**。
   */
  canPin?: boolean
