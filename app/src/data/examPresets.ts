/* ============================================================
   ...
   ⚠️ 第三条（**用户 2026-09-25 拍板**）：**语文的「断句」算客观题**。
      它在卷面上就是涂卡作答（第 10 题：三处各 1 分，共 3 分），所以
      `examPresets` 里按 `kind: 'single'` 录入 —— 判分走多选 `m/n`（每涂对一处得 1 分），
      与卷面规则一致。于是**语文客观题 = 10 道单项选择 + 断句 = 11 题 · 33 分**
      （主观 117 分）。⚠️ 断句**已经在这 11 题里**，别再"再加 3 分"算成 36 —— 那是算两遍。
      逐题核对记录见 `功能设计与不变量.md` §14.11。
   ============================================================ */
...
export type PresetQuestion = {
  ...
  each: number
  /**
   * **卷面上**是不是客观题（机器可判：涂卡 / 选项）。
   *
   * ⚠️ 它**只描述卷面**，不是判分判据 —— 平台上"客观题得分"一律看 `kind`
   *    （`lib/examPaper.ts` 的 `objectiveSubjective` 走 `isChoiceKind`）。
   *    两者**故意不一定一致**：
   *     · 英语的听力与阅读理解卷面上是客观题，但平台上按 `listen` / `reading`
   *       录入（`objective: true`，却不算进"客观题得分"）；
   *     · 语文的「断句·涂卡」卷面上是客观题，**平台上也算客观题** ——
   *       用户 2026-09-25 拍板，所以它按 `single` 录入（见本文件开头第三条）。
   */
  objective?: boolean
