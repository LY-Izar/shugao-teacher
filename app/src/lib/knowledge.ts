/* ============================================================
   高中物理知识体系 + 自动打标
   ------------------------------------------------------------
   🔴 **这棵树今天只有物理一棵**（多学科体系方案 §3.2 第二阶段才会做成
      「学科 → 教材版本 → 章节 → 知识点」四张表 + 每个学科一棵树）。
      在第一阶段（学科维度）里这里**一行都不动** ——
      多学科的坑不在"树只有一棵"，而在 `POINT_NAME` / `POINT_CHAPTER`
      是**两张全局表**：第二个学科一旦定义了同名 id（比如语文也写 `energy`），
      物理与语文会互相覆盖显示名。这是第二阶段第一件要换掉的东西。
