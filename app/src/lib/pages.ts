 ...
 *
 * 🔴 **这一组数字一个字都不要动** —— 它是"**6 档教师身份** × 34 行"那个口径的自证值
 *    （`按身份显示导航方案.md` §2.2）。`管理架构与角色权限方案.md` §四.0 用脚本重算过，
 *    逐项相等（145 / 27 / 32 与那份文档的"规模感"表一致）。
 *    ⚠️ 2026-09-28 加了通知两行之后，**它仍然是 34** —— 因为它量的是
 *    **那一份文档的矩阵**，而不是 `PAGES.length`。
 *    （D2 的断言口径随之从"`PAGES.length === rows`"改成"矩阵自己必须是 34 行 +
 *      通知两行必须是新加的第 35 / 36 行"两句话，见 `nav-checks.mjs` 的 D2。）
 *    新口径的数字在下面 `MATRIX_SHAPE_13` 那一组 —— **另加一组，不改这一组**。
 */
