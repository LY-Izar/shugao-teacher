...
export const PAGES: PageRow[] = [
   ...
   *
   * 🔴 **它的地址不叫 `/admin`**：`/admin` 已经被「平台运维」（超管专属、不套 `Guard`、
   *    不套 `AppShell` 的那条线）占着，而那两件事完全不是一回事 ——
   *    `/manage` 是**行政事务**（教务处 / 年级主任 / 办公室主任这一层），
   *    `/admin` 是**平台维护**（只有超管）。名字像但读者与职责都不同，混用会让人
   *    在你点进去之前分不清哪一个是"给教务处用的"。
   *
   * ⚠️ 这一条是**真路由**（不是 ★）：页面做出来了（`pages/Administration.tsx`），
   *    所以 `PLANNED_PAGE_COUNT` **不动**（仍是那一条 `/admin/probes`）。
   * 🔴 它进 `NAV`（桌面左栏 / 移动端展开层）：它是**一个页面**，不是「我的」里的一行设置项。
   *    入口判据 = 那三张卡各自判据的**并集**（今天 == `canManageTeachers`，推导见
   *    `roles.ts` 的 `seesAdministration()`）。
   *
   * ⚠️ 它是**矩阵要加行**的那一类（新地址），与 P6/P3 那几次"从 ★ 变真路由"不同：
   *    13 列矩阵（`管理架构与角色权限方案.md` §4.2）与 6 列矩阵（`按身份显示导航方案.md` §2.2）
   *    里**原来都没有这一行** → 两份文档各加一行，`MATRIX_SHAPE` / `MATRIX_SHAPE_13` 跟着改。
   */
  { path: '/manage', group: 'NAV（行政管理）', entry: '/manage' },
 ...
 *
 * 🔴 **2026-10-01「行政管理」（`/manage`）落地：这一组真的变了**
 *    （**35 / 148 / 29 / 33** —— 加行前按同一口径读回来是 **34 / 147 / 28 / 33**），
 *    因为这一轮是**加了一个新地址**（不是去掉 ★）：`按身份显示导航方案.md` §2.2 的表里
 *    原来**没有** `/manage` 这一行，本轮加了一行（那一行对超管 / 教务处 / 年级主任是 V、
 *    对另外五档是 E、教室端是 B）。
 *
 *    🔴 **同时纠正一处旧的手算误差**：旧文档与旧代码写的是 `145 / 27 / 32`，
 *       而**按 §2.2 逐行读回来**（本轮用核对脚本 + `nav-checks` 打印的"实测"两边对过）
 *       那一版就是 `147 / 28 / 33` —— **差 3 格**。最明显的是 `B`：
 *       教室端那一列**整列都是 `B`**（只有 `/login` 与 `/classroom` 两行例外），
 *       行数一变它就跟着变，手算很容易漏一行。
 *       `管理架构与角色权限方案.md` §4.1 与 `按身份显示导航方案.md` §2.2 的"规模感"行
 *       本轮**一起按实测改了**。
 *       → 又一次现场：**写死的聚合数字会错，以逐行读回来的为准**（§一 那条纪律）。
 *    ⚠️ 与上面 P6 那条教训**不冲突**：那一条说的是"地址原来就在表里 → 不加行"，
 *       本轮是"地址本来就**不在**表里 → 必须加行"。判据永远是"矩阵里有没有这一行"。
 */
 ...
 *   ① 原矩阵        34 × 6  = 204  → V147 / E28 / **B33**   ← 加 `/manage` 之前（按 §2.2 读回来）
 *   ①′ 加了通知两行  36 × 6  = 216  → **V156 / E30 / B35**
 *   ② 本方案的矩阵   37 × 13 = 481  → **V349 / E81 / B35 / —16**   ← 2026-10-01 加 `/manage` 之后
 *   ⚠️ 方案原文手算的 `145 / 27 / 32`、`153 / 29 / 34`、`345 / 73 / 34 / 16` 都与逐行读回来的
 *      实测**差若干格** —— 那几处旧的聚合数本轮按实测改正（见 §4.1 的说明）。
 ...
 *    🔴 **2026-10-01 更正：实测（`_crop` 里按 D9 的口径逐行读回来）是 16，不是 17** ——
 *       办公室主任那一列今天有 **17 格 V**（`/notices` 与 `/notices/new` 那两格 + 行 28 的 `/settings`），
 *       其余 19 行是 `—`。上面那句"16 变 17"是上一轮**没落到实测**的一处笔误，本轮按实测落 16。
 *       ⚠️ `/manage` 那一行他是 **V**（三张卡里有两张对他摆）→ 这一列 `V 19 → 20`、`—` 仍是 16。
 *
 * 🔴 **2026-10-01「行政管理」（`/manage`）落地：这一组变了**（36/345/73/34/16 →
 *    **37/348/74/34/16**），因为这一轮往 §4.2 的 13 列表里**加了一行**（新地址 `/manage`）：
 *      · `rows 36 → 37`；
 *      · `v 345 → 348`：超管 / 教务处 / 办公室主任三列各 +1；
 *      · `e 73 → 74`：只有**年级主任那一列** +1（他对 `/grades` 是 V，对 `/manage` 这个入口是 E）——
 *        ⚠️ 不是"+1 一列"，是"矩阵里那一行的格子里只有一个 E"。逐列核对 `perColumn` 就是干这个的。
 *      · `b 34 → 34`（教室端那一格照旧是 B）；`— 16 → 16`（办公室主任那一格是 V，不是 `—`）。
 *    ⚠️ 两只口径**分开量**：6 列那一组是 35/146/28/32（见上面），与这一组**不许互相推导**。
 */
export const MATRIX_SHAPE_13 = {
  rows: 37,
  v: 349,
  /** ⚠️ 方案原文写 60，实际 73（见上面那段改正说明）；2026-10-01 加 `/manage` 一行后 81 */
  e: 81,
  b: 35,
  /** `—` 不适用（只有办公室主任那一列有） */
  excluded: 16,
   ...
   *
   * 🔴 2026-10-01 加 `/manage` 那一行 + **按 §4.2 逐行读回来改正**：
   *    `/manage` 那一行对超管 / 教务处 / 办公室主任 / 年级主任是 V、其余八列是 E、教室端是 B
   *    → 超管 / 教务处 / 办公室主任 / 年级主任各 +1，其余列不动（`V349 / E81 / B35`）。
   *    ⚠️ 旧的 `345 / 73` 与这里的若干列（校级三档 / 德育处是 30/6）**与表格对不上**，
   *       本轮按逐行读回来的实测落 —— 这也是为什么这一节的值必须**跑一次 `nav-checks`
   *       把"实测"抄回来**，而不是按"我改了什么"去推。
   */
  perColumn: [
    { v: 36, e: 1, b: 0, x: 0 },
    { v: 34, e: 3, b: 0, x: 0 },
