/* ============================================================
   ...
   ① **走班科目怎么算**（`streamDiff` / `planStreamClasses`）
      🔴 **走班班按科目建**（2026-10-06 用户拍板 = C 方案）：每个走班科目**最多一个班**，
      一个学生的 `walk` 里有几门就进几个班（U-1 = A 的多对多 —— 一个学生**可以同时**在 2 个班里）。
      `物政地` 的学生（理科班，默认物化生）差 **2 门**：他要走的是 **政治 + 地理**
      （本班默认教的化学 / 生物他不上）→ 进「政治走班班」+「地理走班班」两个班。
 ...
 * 能走班的四科 —— **全仓唯一的定义处**。
 *
 * 顺序 = 走班班 / 组合名的显示顺序（`物化政` 这种要读得出来），
 * ⚠️ 它不是"随便排的"：换顺序会让走班班的**排列顺序**与多科键的串变样 → 生成结果不再逐字相同。
 */
...
/**
 * 多科键的分隔符。⚠️ **生成建议今天只产出单科键**（`politics`）——
 * 这个分隔符留给"手工建过 / 老库里留下的多科走班班"：`generate_stream_classes()`、
 * `assign_stream_teacher()` 与 `streamSubjectsOf()` 都照它拆科目（认得出就不算坏数据）。
 */
export const STREAM_KEY_SEP = '+'

/**
 * 走班班的键（`classes.stream_key`）= **这个班教哪几科的科目代码**。
 * 🔴 按科目建班之后它是**单科代码**（`politics`）；只有手工建的多科班才会出现 `a+b`。
 * **顺序固定**（按 `STREAM_SUBJECT_CODES`），不许就地拼。
 */
...
/** 走班班的显示名（Q14：走班班自带号，**不用真实教室名**）—— 单科班长成「走班班-政治」 */
 ...
 *
 * 🔴 **分组口径 = 按科目**（2026-10-06 用户拍板 = C）：每个走班科目**最多一个班** ——
 *    **不是**"按 `walk` 集合分组"。三个学生分别选物政地 / 物化政 / 物政生，`walk` 集合
 *    并不相同（`{政治,地理}` / `{政治}` / `{政治}`），但**都要上政治的都在同一个政治班**里。
 *    这条是"班数最少、每班人最多、最像真学校的走班"那条口径的**唯一防线**
 *    （`grade-checks` R37 正向钉 + R38 反向对照）。
